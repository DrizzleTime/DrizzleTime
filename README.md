# 👋 Hi there, I'm Shiyu (DrizzleTime)

💡 Enthusiastic open-source contributor & lifelong tech learner  
🧠 Always exploring, building, and sharing knowledge  
⚡ Enjoy clean code, smart automation, and solving real-world challenges  
👾 Working with C#, TypeScript, and Python — from backend to frontend to AI tinkering  
🌱 Curious about new tech: AI, full-stack, cloud, and more  
🤝 Open to collaboration, knowledge exchange, and creative projects  
🌟 Let's create, learn, and grow together!

---

## 🔗 Connect with Me

- 🌐 [Blog](https://shiyu.dev)
- 📧 Email: [im@shiyu.dev](mailto:im@shiyu.dev)

---

## 📊 GitHub Stats & 🚀 Languages I Love

<table>
  <tr>
    <td>
      <img src="https://github-readme-stats.vercel.app/api?username=DrizzleTime&show_icons=true&theme=transparent" alt="DrizzleTime's GitHub Stats" />
    </td>
    <td>
      <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=DrizzleTime&layout=compact&hide=html,css&theme=transparent" alt="Most Used Languages" />
    </td>
  </tr>
</table>
